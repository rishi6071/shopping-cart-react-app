...
import { Link, Grid, Button, Typography, Divider, ButtonGroup } from "@material-ui/core";
...
const ProductDetails = (props) => {
  ...
  useEffect(() => {
    try {
      if (Object.keys(productItem).length !== 0) {
        ...
        p_description = p_description.split("<h2><strong>About this item:</strong></h2>");
  ...
  const handleQty = (operation = "") => {
    ...
    if (operation === "+") {
      if (prodQty < prodInStock) setProdQty((prevState) => prevState + 1);
    } else {
      if (prodQty > 1) setProdQty((prevState) => prevState - 1);
    }
  };
        ...
        <Grid item md={6} sm={10} className={classes.productDetailsItem}>
          ...
          <Grid container style={{ display: "flex", justifyContent: "center" }}>
              ...
              >
                {!loading ? (
                  [...productImgGallery].map((galleryImg) => {
                    return (
                      <>
                        <Grid item sm={12} xs={2} className={classes.prodGalleryImgBox} key={galleryImg.id}>
                          <img src={galleryImg.url} onMouseOver={handleMainImg} alt={galleryImg.id} />
                        </Grid>
                      </>
                    );
                  })
            ...
            <Grid item sm={9} xs={12} className={classes.productMainImgBox}>
              {!loading ? (
                <img src={productMainImg} className={classes.productMainImg} alt="ProductMainImage" />
              ) : (
                <SkeletonProdMainImg />
              )}
            </Grid>
              ...
              <Grid container style={{ display: "flex", justifyContent: "center" }}>
                <Grid item sm={6} xs={11} className={classes.addCartButtonBox}>
                  {!loading ? (
                    ...
                    >
                      <AddShoppingCartIcon className={classes.buttonIcon} /> <span>Add To Cart</span>
                    </Button>
                  ) : (
                    <SkeletonAddBuyBtn />
                  )}
                ...
                <Grid item sm={6} xs={11} className={classes.buyButtonBox}>
                  {!loading ? (
                    <Button contained="filled" className={classes.buyCartButton}>
                      <CreditCardIcon className={classes.buttonIcon} /> <span>Buy Now</span>
                    </Button>
                  ) : (
                    <SkeletonAddBuyBtn />
                  )}
          ...
          {!loading ? (
            <div>
              <Typography className={classes.productQuantityField}>
                ...
                <ButtonGroup size="small" aria-label="small button group" style={{ transform: "translateY(5px)" }}>
                  <Button className={classes.quantityButtons} onClick={() => handleQty("-")}>
                    <RemoveIcon />
                  </Button>
                  <input
              ...
              <Typography className={classes.productAvailability}>
                <strong>Availability:</strong>{" "}
                {prodInStock > 0 ? (
                  <span style={{ color: "green" }}>
                    In Stock {prodInStock < 10 ? `(Only ${prodInStock} Left)` : ``}
                  </span>
                ) : (
          ...
          <Grid container className={classes.productDetailsSections}>
            <Switch>
              ...
              <Route path={"/product/" + id + "/policies"} exact>
                <Grid container direction="column" className={classes.shippingDetailsSection}>
                  <Grid container>
                    <Typography variant="h6" className={classes.sampleListHead}>
                      Returns Policy
                    </Typography>
                    <Typography className={classes.sampleList} style={{ marginTop: 10 }}>
                      You may return most new, unopened items within 30 days of delivery for a full refund. We'll also
                      pay the return shipping costs if the return is a result of our error (you received an incorrect or
                      defective item, etc.).
                    </Typography>
                    <Typography className={classes.sampleList} style={{ marginTop: 15 }}>
                      ...
                      the transit time for us to receive your return from the shipper (5 to 10 business days), the time
                      it takes us to process your return once we receive it (3 to 5 business days), and the time it
                      takes your bank to process our refund request (5 to 10 business days).
                    </Typography>
                    <Typography className={classes.sampleList} style={{ marginTop: 15 }}>
